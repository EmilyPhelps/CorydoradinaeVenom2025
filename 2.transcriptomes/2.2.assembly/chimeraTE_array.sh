...
if [ ! -f "${output}/${id}_${trans_name}_fq.tsv" ]; then
   ...
   # Determine number of files to sample (all if less than 5, else 5)
   sample_count=$(( total_files < 5 ? total_files : 5 ))

   # Get random indices or all if fewer than 5
   mapfile -t random_indices < <(shuf -i 0-$((total_files - 1)) -n "$sample_count")
